...
The Problem
-----------
vgn_env packages (torch 2.5.1+cu121, open3d 0.19.0) are compiled for
Python 3.10. The MuJoCo environment runs Python 3.13. Mixing them in the
same interpreter causes ABI errors.
...
_VGN_ENV_PYTHON = Path("/home/shrikar/anaconda3/envs/vgn_env/bin/python3")
...
class VGNBridge:
    ...
    def __init__(
        ...
        if not VGN_OK:
            ...
            print(f"  [VGN] Ready  model={self.model_path.name}  "
    ...
    def collect_view(
        ...
        self._views.append({
            "depth":   depth,
            "cam_pos": cam_pos,
            "cam_mat": cam_mat.flatten(),   # stored as flat 9-elem
    ...
    def run_inference(self) -> bool:
        ...
        with tempfile.NamedTemporaryFile(suffix=".npz", delete=False,
                                         prefix="vgn_views_") as nf:
            npz_path = nf.name
        ...
        try:
            ...
            if not out:
                print(f"  [VGN] Worker produced no output")
                if result.stderr:
            ...
            print(f"  [VGN] {self._n_candidates} candidate(s)  "
    ...
    def best_grasp_world(
        ...
        rot_mat   — 6-DOF grasp orientation from VGN (3×3 rotation matrix)
        ...
        return pos_world, self._grasp_rot.copy(), self._grasp_width, self._grasp_score
